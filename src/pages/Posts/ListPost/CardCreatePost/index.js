...
function CardCreatePost({ user, actionGetAllPosts }) {
        ...
        <div className={cx("card-create-post")}>
            ...
            >
                <FormCreatePostInModal
                    ...
                    actionGetAllPosts={actionGetAllPosts}
                />
            </Modal>
...
const FormCreatePostInModal = ({ user, actionQuit, actionGetAllPosts }) => {
    const { createPost, uploadSingleImage } = useContext(PostContext);

    const inputRef = useRef();
    const imageRef = useRef();
    const [loadingImage, setLoadingImage] = useState(false);
    const [loadingEventUpload, setLoadingEventUpload] = useState(false);
    ...
    const eventSubmitCreatePost = async (e) => {
        ...
        setLoadingEventUpload(false);
        actionQuit(false);
        actionGetAllPosts();
    };

    const eventCloseModal = () => {
        if (loadingEventUpload) {
            return;
        }
                ...
                <div className={cx("grid-footer")}>
                        ...
                        <div className={cx("list-action")}>
                            <div className={cx("action-import")}>
                                <input
                                    ...
                                    disabled={
                                        loadingImage && loadingEventUpload
                                    }
                                />
                                <label
                    ...
                    <div className={cx("grid-button")}>
                        <button
                            ...
                            disabled={loadingEventUpload}
                        >
                            {loadingEventUpload && <Spinner size="sm" />}
